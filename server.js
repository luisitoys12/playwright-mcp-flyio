...
// Proxy manual para SSE - limpia el token de la URL antes de pasar al MCP
app.use('/sse', auth, (req, res) => {
  // Construir URL limpia sin el param ?token
  ...
  const options = {
    ...
    headers: {
      ...
      // quitar el authorization para que no lo vea el MCP
...
// Proxy manual para /message
app.use('/message', auth, express.raw({ type: '*/*' }), (req, res) => {
  const { token, ...cleanQuery } = req.query;
  const qs = new URLSearchParams(cleanQuery).toString();
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .endpoint{background:#0f1117;border:1px solid #2d3148;border-radius:8px;padding:12px 16px;font-family:monospace;font-size:13px;color:#a5b4fc;word-break:break-all}
...
<body><div class="card">
  ...
  <div class="section"><h2>Endpoint SSE</h2>
    <div class="endpoint">https://playwright-mcp-kus.fly.dev/sse?token=TU_TOKEN</div></div>
  ...
  <div class="auth-note">&#128274; <strong>Auth requerida:</strong><br>
    Header: <code>Authorization: Bearer &lt;token&gt;</code><br>
    O query: <code>?token=&lt;tu-token&gt;</code>
  </div>
